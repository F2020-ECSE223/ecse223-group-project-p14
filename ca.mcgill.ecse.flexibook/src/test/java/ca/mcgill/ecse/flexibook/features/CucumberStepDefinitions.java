...
//		}
	...
	public void attempts_to_change_the_service_in_the_appointment_to_at(String username, String serviceName, String currentDateTime) throws InvalidInputException {
		...
		FlexiBookApplication.setCurrentDate(RegisterTime.getStartDate());
		FlexiBookApplication.setCurrentTime(RegisterTime.getStartTime());
		if(currentAppointment.getTimeSlot().getStartDate().after(FlexiBookApplication.getCurrentDate(true))) {
			currentAppointment.updateContent("", serviceName);
		}
...
//		}
	...
	public void the_service_in_the_appointment_shall_be(String serviceName) {
		...
		for (ComboItem comboItem: currentAppointment.getChosenItems()) {
...
//		}
	...
	public void the_appointment_shall_be_for_the_date_with_start_time_and_end_time(String date, String startTime, String endTime) {
		assertEquals(stringToDate(date), currentAppointment.getTimeSlot().getStartDate());
		assertEquals(stringToTime(startTime), currentAppointment.getTimeSlot().getStartTime());
		assertEquals(stringToTime(endTime), currentAppointment.getTimeSlot().getEndTime());
...
//		}
	...
	public void makes_a_appointment_without_choosing_optional_services_for_the_date_and_time_at(String username, String serviceName, String date, String time, String currentDateTime) {
		...
		// Add by mikewang start ---
		if (RegisterTime.getStartDate().before(timeSlot.getStartDate())) {
		// Add by mikewang end ---
...
///////////////////////////////// below is Jed's Change //////////////////////////////
// I'm using currentAppointment instead, not sure if we need to use another varible
// 		List<Appointment> deleteAppointment = new ArrayList<Appointment>();
...
/////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void attempts_to_add_the_optional_service_to_the_service_combo_in_the_appointment_at(String customer, String optserviceName, String dateAndTime) {
		...
		// Add by mikewang start ---
		currentAppointment.updateContent("add",  optserviceName);
		// Add by mikewang end ---
...
//		flexiBook.getAppointments().get(appNumber-1).updateContent("add",  optserviceName);
	...
	public void the_service_combo_shall_have_selected_services(String itemStringList) {
		// Add by mikewang start ---
		List<String> itemList = comboString2List(itemStringList);
		List<String> actualList = new ArrayList<>();
		for (ComboItem items: currentAppointment.getChosenItems()) {
			actualList.add(items.getService().getName());
		}
...
///////////////////////////////// below is Jed's Change //////////////////////////////
// Jed if yours is working, you can keep it i guess, i just happened to modifiy this part today.
// Mine works fine, just having small issues as i posted in facebook chat
// =======
...
// 	}
	...
	private List<String> comboString2List(String comboString){
		String[] sepComboString = comboString.split("\\,");
		List<String> ComboList = new ArrayList<>();
		for (String comboStrings: sepComboString) {
			ComboList.add(comboStrings);
		}
		return ComboList;
